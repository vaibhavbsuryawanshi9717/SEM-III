...
//printf("pas 2 %d\n",err); 
  if ((bind
       (sock->reciver_fd, (struct sockaddr *) &sock->reciver_addr,
	sizeof (sock->reciver_addr))) == -1)
    {
      printf ("pas 2 %i\n", errno);
...
int
forwardMsg (reciverSocketData * sock, char *destNumber, char *fwdmsg)
{
  ...
  int n =
    sendto (sock->reciver_fd, fwdmsg, strlen (fwdmsg), 0,
	    (struct sockaddr *) &sock->forward_addr,
	    sizeof (sock->forward_addr));
  return n;
}
  ...
  for (;;)
    {
      ...
      memset (msg, '\0', 1024);
      recvfrom (sock->reciver_fd, msg, 1024, 0,
	  ...
	  if (pid == 0)
	    {
	      ...
	      if (n > 0)
		sendto (sock->reciver_fd, ok, strlen (ok), 0,
			(struct sockaddr *) &sock->forward_addr,
			sizeof (sock->forward_addr));
	    }
