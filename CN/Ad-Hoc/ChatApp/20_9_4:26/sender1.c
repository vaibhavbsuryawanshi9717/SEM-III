...
clientSocketData *
clientSocket (char *ipaddr, int port)
{
...
onlineuser ()
{
  ...
  FILE *ifp;
  char interface[200] =
    "sudo arp-scan --interface wlo1 -l | grep 192.*.*.* | ";
...
main ()
{
  ...
  for (;;)
	...
	{
	  printf ("\tSending....\n");
	  sendto (sock->conn_fd, buff, strlen (buff), 0,
		  (struct sockaddr *) &sock->listener_addr,
		  sizeof (sock->listener_addr));
