...
main (int argc, char **argv)
{
  int sfd, fd;
  char buf[BUFFERT];
  off_t m, sz;			//when size is more bigger
  int count = 0;
  long int n;
  ...
  if (argc != 4)
    {
      ...
      return EXIT_FAILURE;	//for failure case we can use exit(1) but EXIT_FAILURE is perfectly/appropriate
    }
...
{
  int l, sfd;
  sfd = socket (AF_INET, SOCK_DGRAM, 0);	//socket <- create a new communication endpoint 
  //family , type , protocol    SOCK_DGRAM <- unreliable, connectionless, messages of maximum length
  if (sfd == -1)		//upon failure returns -1
    {
  ...
  bzero (&sock_serv, l);	//its looklike memset for 
