jQuery(document).ready(function($){
	...
	var no = url.lastIndexOf("/");
	var book =url.substr(no+1);
	$( ".cd-images-list  li" ).filter(function( index ) { 
		var title = $(this).find("a h2").text();
//		book = book.replace("-", " ");
		title = title.replace(" ", "-");
//alert(book+' '+title);
		if(title==book){
			$(".cd-images-list li").removeClass("is-selected");
			$(this).addClass("is-selected");
			$(".cd-content-block li").removeClass("is-selected");
			$("#ch-"+index+"").addClass("is-selected");
			if(index>0){
                        	$(".cd-prev").removeClass("inactive");
                        	$(".cd-next").addClass("inactive");
                        }
		}
  });
